...
RUN_ID="$(date +%Y%m%d_%H%M%S)_lacam0_focus5_map_aware_suite"
...
run_case_row() {
  ...
  out="/tmp/lacam0_mapaware_suite_${RANDOM}_${RANDOM}.txt"
...
run_map_chunk() {
  ...
  tmp_dir="$(mktemp -d /tmp/lacam0_mapaware_suite_chunk_XXXXXX)"
  chunk_tmp="${chunk}.tmp"
...
args_for_cfg_and_map() {
  ...
  case "$cfg" in
    ...
    mapaware_random_x35)
      case "$map_name" in
        random-32-32-20.map) mask_args 35 ;;
        *) baseline_args ;;
      esac
      ;;
    *)
