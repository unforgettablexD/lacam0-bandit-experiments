...
POLICIES = ["baseline_x00", "ucb1_x32", "thompson_x32", "epsilon_x32", "random_x32"]
...
def main():
    if len(sys.argv) != 2:
        print("Usage: summarize_focus4_policy_suite_x32.py <raw_csv>", file=sys.stderr)
        sys.exit(1)
    ...
    out_csv = raw_csv.with_name(raw_csv.stem + "_mapwise.csv")
    ...
    for m, n in MAPS:
        ...
        for p in POLICIES:
            c = [r for r in rows if r["config"] == p and r["map"] == m and int(r["agents"]) == n]
            csolved = sum(int(r["solved"]) for r in c)
            cavg = solved_only_avg(c)
            gain = None
            if bavg is not None and cavg is not None and bavg > 0:
            ...
            out_rows.append(
                {
                    ...
                    "config_solved": f"{csolved}/{len(c)}" if c else "0/0",
                    "baseline_avg_soc_solved_only": "" if bavg is None else f"{bavg:.2f}",
                    "config_avg_soc_solved_only": "" if cavg is None else f"{cavg:.2f}",
    ...
    with out_md.open("w") as f:
        f.write("# lacam0 Focus4 X32 Policy Suite\n\n")
        f.write(f"- Raw CSV: `{raw_csv}`\n")
        f.write(f"- Mapwise CSV: `{out_csv}`\n\n")
        f.write("| Map | Agents | Config | Baseline Solved | Config Solved | Baseline Avg SOC (Solved-Only) | Config Avg SOC (Solved-Only) | SOC Gain vs Baseline |\n")
        f.write("|---|---:|---|---:|---:|---:|---:|---:|\n")
        for r in out_rows:
            f.write(
                f"| {r['map'].replace('.map','')} | {r['agents']} | {r['config']} | {r['baseline_solved']} | "
                f"{r['config_solved']} | {r['baseline_avg_soc_solved_only'] or 'NA'} | "
                f"{r['config_avg_soc_solved_only'] or 'NA'} | "
